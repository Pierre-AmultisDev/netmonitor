...
VALUES
    ('thresholds.brute_force.exclude_streaming', 'true'::jsonb, 'bool', NULL, 'global', 'Default value from config.yaml', 'system', NOW()),
    ('thresholds.brute_force.exclude_cdn', 'true'::jsonb, 'bool', NULL, 'global', 'Default value from config.yaml', 'system', NOW())
...
INSERT INTO sensor_configs (parameter_path, parameter_value, parameter_type, sensor_id, scope, description, updated_by, updated_at)
VALUES
    ('thresholds.modern_protocols.quic_detection', 'true'::jsonb, 'bool', NULL, 'global', 'Default value from config.yaml', 'system', NOW()),
    ('thresholds.modern_protocols.http3_detection', 'true'::jsonb, 'bool', NULL, 'global', 'Default value from config.yaml', 'system', NOW())
ON CONFLICT (sensor_id, parameter_path)
DO UPDATE SET
    parameter_value = EXCLUDED.parameter_value,
    updated_at = NOW(),
